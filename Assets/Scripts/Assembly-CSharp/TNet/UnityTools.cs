...
namespace TNet
{
	public static class UnityTools
	{
		...
		private static void PrintException(Exception ex, CachedFunc ent, int funcID, string funcName, params object[] parameters)
		{
			...
			{
				string text4 = text3;
				text3 = string.Concat(text4, ent.obj.GetType(), ".", funcName);
			}
		...
		public static bool ExecuteFirst(List<CachedFunc> rfcs, byte funcID, out object retVal, params object[] parameters)
		{
			...
			for (int i = 0; i < rfcs.size; i++)
			{
				...
				if (ent.id == funcID)
				{
					...
					try
					{
						retVal = ((ent.parameters.Length != 1 || ent.parameters[0].ParameterType != typeof(object[])) ? ent.func.Invoke(ent.obj, parameters) : ent.func.Invoke(ent.obj, new object[1] { parameters }));
						return retVal != null;
					}
					catch (Exception ex)
					{
		...
		public static bool ExecuteAll(List<CachedFunc> rfcs, byte funcID, params object[] parameters)
		{
			for (int i = 0; i < rfcs.size; i++)
			{
				...
				try
				{
					...
					{
						ent.func.Invoke(ent.obj, new object[1] { parameters });
					}
					else
					{
		...
		public static void Broadcast(string methodName, params object[] parameters)
		{
			...
			for (int num = array.Length; i < num; i++)
			{
				...
				if (method != null)
				{
					...
					{
						Debug.LogError(string.Concat(ex.InnerException.Message, " (", monoBehaviour.GetType(), ".", methodName, ")\n", ex.InnerException.StackTrace, "\n"), monoBehaviour);
					}
				}
			}
		}
