...
namespace TNet
{
	public class UdpLobbyServer : LobbyServer
	{
		...
		public override int port
		{
			get
			{
				return mUdp.isActive ? mUdp.listeningPort : 0;
			}
		}
		...
		public override bool isActive
		{
			get
			{
				return mUdp != null && mUdp.isActive;
			}
		}
		...
		private bool ProcessPacket(Buffer buffer, IPEndPoint ip)
		{
			...
			switch ((Packet)binaryReader.ReadByte())
			{
			...
			case Packet.RequestRemoveServer:
			{
				...
				IPEndPoint ip2;
				Tools.Serialize(binaryReader, out ip2);
				IPEndPoint ip3;
				Tools.Serialize(binaryReader, out ip3);
