...
namespace TNet
{
	...
	public class DataNode
	{
		...
		public Type type
		{
			get
			{
				return (value == null) ? typeof(void) : mValue.GetType();
			}
		}
		...
		public T Get<T>(T defaultVal)
		{
			...
			object obj = Get(typeof(T));
			return (mValue == null) ? defaultVal : ((T)obj);
		}
		...
		private static void Write(StreamWriter writer, int tab, string name, object value, bool writeType)
		{
			...
			if (type == typeof(int) || type == typeof(float) || type == typeof(uint) || type == typeof(byte) || type == typeof(short) || type == typeof(ushort))
			{
				if (name != null)
				{
					writer.Write(" = ");
			...
			if (value is TList)
			{
				...
				if (tList.Count > 0)
				{
					...
					{
						Write(writer, tab + 1, null, tList.Get(i), false);
					}
				}
				return;
			}
			if (value is IList)
			{
				...
				if (list.Count > 0)
				{
					...
					{
						Write(writer, tab + 1, null, list[j], false);
					}
				}
				return;
			}
			...
			for (int l = 0; l < serializableFields.size; l++)
			{
				...
				if (obj != null)
				{
					Write(writer, tab + 1, fieldInfo.Name, obj, true);
				}
			}
		}
				...
				if (!type.IsSubclassOf(typeof(Component)))
				{
					...
					if (flag2)
					{
						...
						if (genericArgument != null)
						{
							for (int i = 0; i < children.size; i++)
							{
								DataNode dataNode = children[i];
								if (dataNode.value == null)
								{
									dataNode.mValue = dataNode.name;
									dataNode.mResolved = false;
									dataNode.ResolveValue(genericArgument);
									tList.Add(dataNode.mValue);
								}
								else if (dataNode.name == "Add")
								{
									dataNode.ResolveValue(genericArgument);
									tList.Add(dataNode.mValue);
								}
								...
								{
									UnityEngine.Debug.LogWarning("Unexpected node in an array: " + dataNode.name);
								}
							}
							return false;
						}
					...
					else if (flag)
					{
						...
						if (type2 != null)
						{
							...
							{
								DataNode dataNode2 = children[j];
								if (dataNode2.value == null)
								{
									dataNode2.mValue = dataNode2.name;
									dataNode2.mResolved = false;
									dataNode2.ResolveValue(type2);
									if (flag3)
									{
										list[j] = dataNode2.mValue;
									}
									else
									{
								...
								{
									UnityEngine.Debug.LogWarning("Unexpected node in an array: " + dataNode2.name);
								}
							}
							return false;
						}
					...
					else
					{
						if (type.IsClass)
						{
							...
							{
								DataNode dataNode3 = children[k];
								mValue.SetSerializableField(dataNode3.name, dataNode3.value);
							}
							return false;
						}
		...
		private static Color32 ParseColor32(string text, int offset)
		{
			...
			byte a = (byte)((offset + 8 > text.Length) ? 255u : ((uint)((HexToDecimal(text[offset + 6]) << 4) | HexToDecimal(text[offset + 7]))));
