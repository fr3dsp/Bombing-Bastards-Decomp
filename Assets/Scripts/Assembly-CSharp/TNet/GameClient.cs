...
namespace TNet
{
	public class GameClient
	{
		...
		public long serverTime
		{
			get
			{
				return mTimeDifference + DateTime.UtcNow.Ticks / 10000;
			}
		}
		...
		public int hostID
		{
			get
			{
				return (!mTcp.isConnected) ? mTcp.id : mHost;
			}
		}
		...
		public bool isHosting
		{
			get
			{
				return !mIsInChannel || mHost == mTcp.id;
			}
		}
		...
		public IPEndPoint packetSource
		{
			get
			{
				return (mPacketSource == null) ? mTcp.tcpEndPoint : mPacketSource;
			}
		}
		...
		private bool ProcessPacket(Buffer buffer, IPEndPoint ip)
		{
			...
			if (packet == Packet.ResponseID || mTcp.stage == TcpProtocol.Stage.Verifying)
			{
				...
				{
					mTimeDifference = binaryReader.ReadInt64() - DateTime.UtcNow.Ticks / 10000;
					if (mUdp.isActive)
					{
			...
			OnPacket value;
			if (packetHandlers.TryGetValue((byte)packet, out value) && value != null)
			{
			...
			{
				int milliSeconds = (int)(mMyTime - mPingTime);
				if (ip != null)
				{
			...
			case Packet.Error:
				...
				{
					onConnect(false, binaryReader.ReadString());
				}
			...
			case Packet.ResponseLoadFile:
			{
				...
				OnLoadFile onLoadFile = mLoadFiles.Pop();
				if (onLoadFile != null)
				{
