...
namespace TNet
{
	public class GameServer : FileServer
	{
		...
		public int playerCount
		{
			get
			{
				return isActive ? mPlayers.size : 0;
			}
		}
		...
		public void Update()
		{
			if (mThread == null && mListener != null)
			{
				ThreadFunction();
			}
		}
						...
						if (buffer.size > 0)
						{
							...
							else if (buffer.size > 4)
							{
								...
								try
								{
									...
									packet = (Packet)binaryReader.ReadByte();
									switch (packet)
									{
								...
								{
									Error(string.Concat("(ThreadFunction Read ", packet, ") ", ex2.Message));
									RemovePlayer(player);
								}
							}
						}
					...
					while (num < mPlayers.size)
					{
						...
						for (int i = 0; i < 100; i++)
						{
							...
							if (buffer.size > 0)
							{
								try
								{
									if (ProcessPlayerPacket(buffer, tcpPlayer, true))
									{
										flag = true;
									}
								}
								...
								{
									Error("(ThreadFunction Process) " + ex3.Message);
