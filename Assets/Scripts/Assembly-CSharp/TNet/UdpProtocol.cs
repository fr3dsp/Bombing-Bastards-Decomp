	...
	public class UdpProtocol
	{
		...
		private void OnReceive(IAsyncResult result)
		{
			...
			if (num > 4)
			{
				...
				buffer.BeginWriting(false).Write(mTemp, 0, num);
				buffer.BeginReading(4);
				Datagram item = new Datagram
				{
					buffer = buffer,
					ip = (IPEndPoint)mEndPoint
				};
				lock (mIn)
				{
					mIn.Enqueue(item);
		...
		private void OnSend(IAsyncResult result)
		{
			...
			{
				num = 1;
				Debug.Log("[TNet] OnSend: " + ex.Message);
			}
			lock (mOut)
			{
				...
				{
					Datagram datagram = mOut.Peek();
					mSocket.BeginSendTo(datagram.buffer.buffer, datagram.buffer.position, datagram.buffer.size, SocketFlags.None, datagram.ip, OnSend, null);
				}
			}
		}
