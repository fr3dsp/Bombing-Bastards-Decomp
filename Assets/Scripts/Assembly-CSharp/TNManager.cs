...
{
	public static GameClient.OnPlayerSync onPlayerSync;
	...
	public static int ping
	{
		get
		{
			return (mInstance != null) ? mInstance.mClient.ping : 0;
		}
	}
	...
	public static int listeningPort
	{
		get
		{
			return (mInstance != null) ? mInstance.mClient.listeningPort : 0;
		}
	}
	...
	public static long serverTime
	{
		get
		{
			return (!(mInstance != null)) ? (DateTime.UtcNow.Ticks / 10000) : mInstance.mClient.serverTime;
		}
	}
	...
	public static int channelID
	{
		get
		{
			return isConnected ? mInstance.mClient.channelID : 0;
		}
	}
	...
	public static DataNode playerDataNode
	{
		get
		{
			...
			if (dataNode == null)
			{
				dataNode = (DataNode)(playerData = new DataNode("Version", 12));
			}
			return dataNode;
		}
	}
	...
	private void Awake()
	{
		...
		gameClient9.onPlayerSync = (GameClient.OnPlayerSync)Delegate.Combine(gameClient9.onPlayerSync, new GameClient.OnPlayerSync(OnPlayerSync));
		GameClient gameClient10 = mClient;
		gameClient10.onRenamePlayer = (GameClient.OnRenamePlayer)Delegate.Combine(gameClient10.onRenamePlayer, new GameClient.OnRenamePlayer(OnRenamePlayer));
		GameClient gameClient11 = mClient;
		gameClient11.onCreate = (GameClient.OnCreate)Delegate.Combine(gameClient11.onCreate, new GameClient.OnCreate(OnCreateObject));
		GameClient gameClient12 = mClient;
		gameClient12.onDestroy = (GameClient.OnDestroy)Delegate.Combine(gameClient12.onDestroy, new GameClient.OnDestroy(OnDestroyObject));
		GameClient gameClient13 = mClient;
		gameClient13.onForwardedPacket = (GameClient.OnForwardedPacket)Delegate.Combine(gameClient13.onForwardedPacket, new GameClient.OnForwardedPacket(OnForwardedPacket));
	}
	...
	private static GameObject CreateGameObject(GameObject prefab, BinaryReader reader)
	{
		if (prefab != null)
		{
			...
			object retVal;
			UnityTools.ExecuteFirst(GetRCCs(), b, out retVal, array);
	...
	private void OnForwardedPacket(BinaryReader reader)
	{
		uint objID;
		byte rfcID;
		TNObject.DecodeUID(reader.ReadUInt32(), out objID, out rfcID);
		if (rfcID == 0)
		{
