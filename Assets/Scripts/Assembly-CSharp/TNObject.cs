...
public sealed class TNObject : MonoBehaviour
{
	...
	public uint uid
	{
		get
		{
			return (!(mParent != null)) ? ((uint)id) : mParent.uid;
		}
		set
		{
			if (mParent != null)
	...
	public bool isMine
	{
		get
		{
			return (mOwner != -1) ? (mOwner == TNManager.playerID) : TNManager.isThisMyObject;
		}
	}
	...
	private void RebuildMethodList()
	{
		...
		for (int num = componentsInChildren.Length; i < num; i++)
		{
			...
			{
				if (methods[j].IsDefined(typeof(RFC), true))
				{
					...
					RFC rFC = (RFC)item.func.GetCustomAttributes(typeof(RFC), true)[0];
					item.id = rFC.id;
					mRFCs.Add(item);
				}
			}
	...
	{
		SendRFC(rfcID, null, target, true, objs);
	}

	public void Send(string rfcName, Target target, params object[] objs)
	{
		SendRFC(0, rfcName, target, true, objs);
	}
	...
	public void Send(byte rfcID, Player target, params object[] objs)
	{
		...
		{
			SendRFC(rfcID, null, Target.All, true, objs);
		}
	}
	...
	public void SendQuickly(byte rfcID, Player target, params object[] objs)
	{
		...
		{
			SendRFC(rfcID, null, Target.All, false, objs);
		}
	}
